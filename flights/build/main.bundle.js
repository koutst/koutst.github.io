...
(function () {
  ...
  function quat2euler(t) {
    ...
    return [Math.atan2(2 * (t[0] * t[1] + t[2] * t[3]), 1 - 2 * (t[1] * t[1] + t[2] * t[2])) * toDeg, Math.asin(Math.max(-1, Math.min(1, 2 * (t[0] * t[2] - t[3] * t[1])))) * toDeg, Math.atan2(2 * (t[0] * t[3] + t[1] * t[2]), 1 - 2 * (t[2] * t[2] + t[3] * t[3])) * toDeg];
  }
  ...
  */
  ...
  var drag = d3.drag().on("start", dragstart).on("drag", dragged).on("end", dragend);
  ...
  function dragstart() {
    ...
    debugger;
    svg.insert("path").datum({ type: "Point", coordinates: gpos0 }).attr("class", "point").attr("d", path);
  }
  ...
  var rectZoom = svg.append("rect").data([projection]).attr("class", "rect-zoom").attr("height", height + margin.top + margin.down).attr("width", width + margin.left + margin.right).style("fill", "none").style("pointer-events", "all").call(drag).call(zoom);
  // .call(zoom)
  ...
  function ready(error, countriesData, airportsData, flightData) {
    ...
    for (var i = 0; i < flightData.length; i++) {
      var orgAir = flightData[i]["ORIGIN"];
      var destAir = flightData[i]["DEST"];
      var depTime = flightData[i]["DEP_TIME"];
